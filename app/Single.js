...
export default function Single({ tracks = [] }) {
  ...
  return (
    <div>
      ...
      <div className="w-full max-w-6xl mx-auto mt-6 px-4 flex flex-col md:flex-row gap-6 text-white">
        {/* لیست ترک‌ها */}
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-1 gap-4 flex-shrink-0 md:w-1/3">
          {topTracks.map((track) => (
            <div
              key={track.id}
            ...
            {selectedTrack ? (
              ...
              >
                ...
                {selectedTrack.preview ? (
                  ...
                  <p className="text-sm text-red-400">🎧 No preview available</p>
                )}
              </motion.div>
            ) : (
              <motion.p
